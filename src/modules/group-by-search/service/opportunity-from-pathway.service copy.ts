...
@Injectable()
export class OpportunityFromPathwayService {
    ...
    async opportunityFromPathway(body: any): Promise<any> {

        let real = await this.stemModal.find({ field: body.data }).then((res: any) => { return res })
        let opportunityIdList = new Set(real.map((item: any) => item.Opportunity))

        let opportunityList = await this.opportunityModal.aggregate([
            {
                $match: {
                    _id: { $in: [...opportunityIdList] }
                },
            }
        ])
        return {
