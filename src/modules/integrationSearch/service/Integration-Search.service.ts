...
export class IntegrationSearchService {
    ...
    async SchoolRealTimeRead(body: any): Promise<any> {
        ...
        let realopportunityIdList = searchParameter.length > 0 && body.RealOpportunity && body.RealOpportunity.length === 0 ? await this.opportunityModal.find({ opportunity: { $in: regexArray } }).lean().select('_id').exec().then((result) => result.map((item) => new ObjectId(item._id))) : []
        ...
        if (body.RealOpportunity && body.RealOpportunity.length > 0) {
